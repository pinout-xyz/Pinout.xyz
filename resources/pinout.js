...
for(const panel of group.querySelectorAll('[role="tabpanel"]')){panel.hidden=panel.id!==tab.getAttribute('aria-controls')}
if(focus){tab.focus()}}
for(const group of document.querySelectorAll('.pin-function-tabs')){const tabs=group.querySelectorAll('[role="tab"]')
...
for(const block of document.querySelectorAll('pre')){block.classList.add('prettyprint','linenums')}
window.prettyPrint?.()
content?.addEventListener('click',event=>{const tab=event.target.closest('[role="tab"]')
...
content?.addEventListener('keydown',event=>{const step={ArrowRight:1,ArrowLeft:-1}[event.key]
...
const highlight=(event,hovered)=>{const reference=event.target.closest('article .pin-hover')
if(!reference){return}
for(const item of pinNav(reference.dataset.pin)){item.classList.toggle('hover-pin',hovered)}}
...
view?.addEventListener('click',event=>{const button=event.target.closest('button')
if(!button){return}
